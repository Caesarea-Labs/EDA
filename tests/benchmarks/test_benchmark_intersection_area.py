...
from eda.utils import equals_roughly, sum_of
...
def shapely_intersect_area_sum(polygon_list: list[Polygon], rect: Polygon) -> float:
    # TODO: when running the algorithm, assert the intersected area is not higher than the area of the rect itself.
    """
    Calculates the area that a rectangle intersects with a list of polygons by summing the areas of the intersections.
    This has the benefit of being faster, but can provide an incorrect result in the case multiple polygons span the same area as the rectangle. 
    For this reason, the polygon list must be pre-processed with remove_polygon_overlaps. 
    """
    intersecting_subset = [polygon for polygon in polygon_list if rect.intersects(polygon)]
    intersections = [polygon.intersection(rect) for polygon in intersecting_subset]
...
def test_benchmark_intersection_area_union():
    benchmark("Shapely intersects area with union", shapely_intersect_union_benchmark, iterations=10000)


def test_benchmark_intersection_area_sum():
    benchmark("Shapely intersects area with sum", shapely_intersect_sum_benchmark, iterations=10000)


def test_benchmark_intersection_area_sum_no_intersects():
    benchmark("Shapely intersects area with sum and no intersects check", shapely_intersect_sum_no_intersects_benchmark, iterations=10000)
